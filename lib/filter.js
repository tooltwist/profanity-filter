...
module.exports = {
   ...
   * @method sanitize
   * @param  {String} string The phrase to be cleaned
   * @return {Object} An object containing the count of words replaced and
                      a string with all words in the dictionary filtered
   */
  sanitize: function (string) {
    var key, keyReplacement, matchCount, regex, index, patt;
    matchCount = 0;
    // loop through each key in the dictionary and search for matches
    for (key in dictionary) {
      patt = new RegExp('\\b' + key + '\\b', "ig");
    ...
    console.log("sanitize result: ", matchCount, string);
    return {
      found: matchCount,
      result: string
    };
